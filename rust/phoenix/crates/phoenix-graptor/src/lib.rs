...
pub fn build_graph_delta(
    ...
    build_graph_delta_from_compact_rows(store, &state, request)
}

fn build_graph_delta_from_compact_rows(
    store: &PhoenixCozoStore,
    state: &SessionState,
    request: &GraphDeltaRequest,
) -> Result<GraphDeltaResult, StoreError> {
    ...
    let allowed_documents = graph_delta_allowed_documents(state, request);
    let mut diagnostics = graph_delta_diagnostics(request);
    let mut vertices = FxHashMap::<String, CompactDeltaVertex>::default();
    for row in store.fetch_compact_rows_with_columns("graph_vertices", VERTEX_COLUMNS)? {
        let row = CompactRowView::new(VERTEX_COLUMNS, &row);
        let Some(vertex) = compact_delta_vertex(&row) else {
            continue;
        };
        vertices.insert(vertex.id.clone(), vertex);
    }

    let mut chunk_ids = vertices
        .values()
    ...
    let mut edges = Vec::<CompactDeltaEdge>::new();
    let mut adjacency = FxHashMap::<String, Vec<usize>>::default();

    append_compact_delta_edges(
        store,
        "graph_edges",
        EDGE_COLUMNS,
        &vertices,
    ...
    while let Some(vertex_id) = traversal_frontier.pop_front() {
        ...
        let Some(edge_indexes) = adjacency.get(&vertex_id) else {
            continue;
        };
        for edge_index in edge_indexes {
            ...
            if !compact_delta_vertex_allowed(vertex, &allowed_documents, &chunk_id_set) {
                continue;
            }
    ...
    for chunk_id in &chunk_ids {
        let Some(vertex) = vertices.get(chunk_id) else {
            continue;
        };
        ...
        chunks.push(GraphDeltaChunk {
            ...
            note_id: vertex.note_id.as_ref().map(|value| NoteId(value.clone())),
    ...
    let mut extra_node_ids = included_nodes
        ...
        .collect::<Vec<_>>();
    extra_node_ids.sort();
    let mut nodes = Vec::with_capacity(extra_node_ids.len());
    for node_id in &extra_node_ids {
        let Some(vertex) = vertices.get(node_id) else {
            continue;
        };
    ...
    for edge in edges {
        if included_nodes.contains(&edge.source_id)
            && included_nodes.contains(&edge.target_id)
            && edge_keys.insert((
        ...
        {
            delta_edges.push(GraphDeltaEdge {
...
#[derive(Clone, Debug)]
struct CompactDeltaVertex {
    id: String,
    kind: String,
    label: String,
    weight: i64,
    entity_id: Option<String>,
    search_chunk_id: Option<String>,
    document_id: Option<String>,
    note_id: Option<String>,
    chapter_id: Option<u32>,
    boundary_id: Option<u32>,
...
fn append_compact_delta_edges(
    store: &PhoenixCozoStore,
    relation: &str,
    columns: &[&str],
    vertices: &FxHashMap<String, CompactDeltaVertex>,
    allowed_documents: &FxHashSet<String>,
    chunk_id_set: &FxHashSet<String>,
    included_nodes: &mut FxHashSet<String>,
    edges: &mut Vec<CompactDeltaEdge>,
    adjacency: &mut FxHashMap<String, Vec<usize>>,
    candidate_only_active: bool,
) -> Result<(), StoreError> {
    for row in store.fetch_compact_rows_with_columns(relation, columns)? {
        let row = CompactRowView::new(columns, &row);
        let Some(edge) = compact_delta_edge(&row, candidate_only_active) else {
            continue;
        };
        let Some(source) = vertices.get(&edge.source_id) else {
            continue;
        };
        let Some(target) = vertices.get(&edge.target_id) else {
            continue;
        };
        if !compact_delta_vertex_allowed(source, allowed_documents, chunk_id_set)
            || !compact_delta_vertex_allowed(target, allowed_documents, chunk_id_set)
        {
            continue;
        }
        if chunk_id_set.contains(&edge.source_id)
            && matches!(target.kind.as_str(), "entity" | "event")
        {
            included_nodes.insert(edge.target_id.clone());
        }
        if chunk_id_set.contains(&edge.target_id)
            && matches!(source.kind.as_str(), "entity" | "event")
        {
            included_nodes.insert(edge.source_id.clone());
        }
...
fn compact_delta_vertex(row: &CompactRowView<'_>) -> Option<CompactDeltaVertex> {
    ...
    let note_id = attributes
        .get("noteId")
        .and_then(Value::as_str)
        .map(str::to_owned);
    Some(CompactDeltaVertex {
        id: id.to_owned(),
        kind,
        label,
        weight: row.get_i64("weight").unwrap_or(1),
...
fn compact_delta_edge(
    row: &CompactRowView<'_>,
    candidate_only_active: bool,
) -> Option<CompactDeltaEdge> {
    let attributes = row.get_json("attributes").unwrap_or(Value::Null);
    if candidate_only_active && !candidate_edge_is_active(&attributes) {
        return None;
    }
    Some(CompactDeltaEdge {
        source_id: row.get_str("source_id")?.to_owned(),
        target_id: row.get_str("target_id")?.to_owned(),
        edge_type: row.get_str("edge_type").unwrap_or("edge").to_owned(),
        weight: row.get_i64("weight").unwrap_or(1),
    })
}

fn graph_delta_allowed_documents(
    state: &SessionState,
    request: &GraphDeltaRequest,
) -> FxHashSet<String> {
...
fn graph_delta_diagnostics(request: &GraphDeltaRequest) -> Vec<Diagnostic> {
    let mut diagnostics = Vec::new();
    if let Some(since_commit) = request.since_commit.as_ref() {
        diagnostics.push(Diagnostic {
            code: "PX_GRAPH_DELTA_COMPACT".to_owned(),
            message: format!(
                "Graph delta used compact relation rows; sinceCommit {} was treated as a hint only.",
                since_commit.0
            ),
        });
    }
...
fn compact_delta_vertex_allowed(
    vertex: &CompactDeltaVertex,
    allowed_documents: &FxHashSet<String>,
    chunk_id_set: &FxHashSet<String>,
) -> bool {
...
mod tests {
    ...
    #[test]
    fn graph_delta_uses_compact_rows() {
        let store = PhoenixCozoStore::new().expect("store");
        let scanner = PhoenixScanner::default();
        let structure = PhoenixStructure::default();
        let graptor = PhoenixGraptor::default();
        let session_id = SessionId("graph-delta-compact".to_owned());
        graptor
            .ingest(
                &store,
                &scanner,
                ...
                &IngestRequest {
                    ...
                    documents: vec![sample_document(
                        "Chapter 1\nRyan found the signal. Ryan carried it home.",
                    )],
                    commit: false,
                },
            )
        ...
        let delta = build_graph_delta(
            &store,
            &GraphDeltaRequest {
                session_id,
                changed_documents: vec![DocumentId("doc-1".to_owned())],
                since_commit: Some(phoenix_types::CommitId("commit-1".to_owned())),
                ..GraphDeltaRequest::default()
            },
        )
        .expect("graph delta");
        ...
        assert!(delta
            .diagnostics
            .iter()
            .any(|diagnostic| diagnostic.code == "PX_GRAPH_DELTA_COMPACT"));
    }
