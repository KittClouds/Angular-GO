...
impl PhoenixGldr {
    ...
    pub fn query(
        ...
        let wants_chunks = request.targets.is_empty()
            || request
                .targets
                .iter()
                .any(|target| matches!(target, phoenix_types::QueryTarget::Chunks | phoenix_types::QueryTarget::Graph));
        ...
        let mut node_scores = BTreeMap::<String, f64>::new();

        for hit in lexical.span_hits.iter().take(self.config.seed_limit.max(limit)) {
            ...
            {
                for chapter_edge in graph.outgoing_matching(
                    &chapter_vertex_id(document_id, chapter_id),
                    "cross_chapter",
                ) {
                    ...
                    for related_leaf in
                        graph.chapter_leaves(target_document_id, target_chapter_id)
                    {
    ...
    fn expand_entity(
        ...
        chunk_scores: &mut BTreeMap<String, f64>,
        node_scores: &mut BTreeMap<String, f64>,
    ) {
...
impl TemporalFilter {
    fn from_marker(marker: Option<&TemporalMarker>) -> Self {
        let chapter = marker
            .and_then(|marker| marker.chapter.or_else(|| marker.ordinal.map(|value| value as u32)));
        Self { chapter }
    }
    ...
    fn matches_vertex(&self, vertex: &GraptorVertex) -> bool {
        match self.chapter {
            None => true,
            Some(chapter) => {
                vertex.chapter_id == Some(chapter) || vertex.chapters.iter().any(|value| *value == chapter)
            }
        }
    }
    ...
    fn diagnostic_message(&self) -> String {
        match self.chapter {
            Some(chapter) => format!("Applied chapter-local temporal filtering at chapter {chapter}."),
            None => "No chapter-local temporal filter was applied.".to_owned(),
        }
    }
}

fn accumulate_score(scores: &mut BTreeMap<String, f64>, key: &str, score: f64) {
    scores
        .entry(key.to_owned())
...
fn ranked_chunk_hits(scores: BTreeMap<String, f64>, limit: usize) -> Vec<ChunkHit> {
    let mut hits = scores
        .into_iter()
...
fn ranked_node_hits(scores: BTreeMap<String, f64>, limit: usize) -> Vec<NodeHit> {
    let mut hits = scores
        .into_iter()
