...
impl OmEngine {
    ...
    pub fn prepare_pending_action_with_graph(
        ...
        if config.graph_index_enabled && config.index_raw_messages {
            ...
            bridge
                .index_message_window(store, thread_id, &source_key, &eligible)
    ...
    pub fn apply_pending_action_with_graph(
        ...
        match action.kind.as_str() {
            OBSERVE_KIND => {
                self.apply_observation_with_graph(store, bridge, config, action, response)
            }
            REFLECT_KIND => {
                self.apply_reflection_with_graph(store, bridge, config, action, response)
            }
            other => Err(OmError::UnsupportedAction(other.to_owned())),
        }
    }
    ...
    fn load_record(
        ...
        Ok(store
            ...
            .into_iter()
            .find(|row| row_str(row, "thread_id") == Some(thread_id))
...
fn record_from_row(row: &Value) -> Result<OmRecord, StoreError> {
    Ok(OmRecord {
        ...
        observations: row_str(row, "observations").unwrap_or_default().to_owned(),
        current_task: row_str(row, "current_task").unwrap_or_default().to_owned(),
        suggested_continuation: row_str(row, "suggested_continuation")
            .map(str::to_owned)
            .filter(|value| !value.is_empty()),
...
fn message_from_row(row: &Value) -> Result<ThreadMessage, StoreError> {
    Ok(ThreadMessage {
        id: row_str(row, "id").unwrap_or_default().to_owned(),
        thread_id: row_str(row, "thread_id").unwrap_or_default().to_owned(),
        role: row_str(row, "role").unwrap_or_default().to_owned(),
        content: row_str(row, "content").unwrap_or_default().to_owned(),
        narrative_id: row_str(row, "narrative_id").unwrap_or_default().to_owned(),
        created_at: row_i64(row, "created_at").unwrap_or_default(),
        updated_at: row_i64(row, "updated_at").unwrap_or_default(),
...
fn row_bool(row: &Value, field: &str) -> Option<bool> {
...
pub(crate) mod test_store {
    ...
    use phoenix_store_native_core::{
        PhoenixNativeRowStore, SnapshotEnvelope, SnapshotPartition, StoreError,
    };
    use serde_json::Value;

    #[derive(Default)]
    pub(crate) struct TestRowStore {
        rows: Mutex<BTreeMap<String, Vec<Value>>>,
    }

    impl PhoenixNativeRowStore for TestRowStore {
        ...
        fn relation_counts(&self) -> Result<Vec<(String, usize)>, StoreError> {
            Ok(self
                .rows
                .lock()
                .expect("test row store poisoned")
                .iter()
                .map(|(relation, rows)| (relation.clone(), rows.len()))
                .collect())
        }
        ...
        fn put_row(&self, relation: &str, row: Value) -> Result<(), StoreError> {
            let mut guard = self.rows.lock().expect("test row store poisoned");
            let rows = guard.entry(relation.to_owned()).or_default();
            let fields = key_fields(relation);
            if !fields.is_empty() {
                rows.retain(|existing| !same_key(existing, &row, fields));
            }
            rows.push(row);
            Ok(())
        }
        ...
        fn replace_relation_rows(&self, relation: &str, rows: &[Value]) -> Result<(), StoreError> {
            self.rows
                .lock()
                .expect("test row store poisoned")
                .insert(relation.to_owned(), rows.to_vec());
            Ok(())
        }

        fn delete_rows(&self, relation: &str, rows: &[Value]) -> Result<usize, StoreError> {
            let mut guard = self.rows.lock().expect("test row store poisoned");
            let Some(existing) = guard.get_mut(relation) else {
                return Ok(0);
            };
        ...
        fn export_snapshot_partition(
            &self,
            _partition: SnapshotPartition,
        ) -> Result<Vec<u8>, StoreError> {
            Ok(Vec::new())
        }

        fn import_snapshot(&self, _bytes: &[u8]) -> Result<SnapshotEnvelope, StoreError> {
            Ok(SnapshotEnvelope {
    ...
    fn key_fields(relation: &str) -> &'static [&'static str] {
        match relation {
            "threads"
            | "thread_messages"
            | "chat_runs"
            ...
            | "om_generations" => &["id"],
            "om_records" => &["thread_id"],
            "om_graph_index" => &["thread_id", "kind", "source_key", "document_id"],
            "om_graph_entities" => &["thread_id", "document_id", "entity_id"],
            "om_graph_relations" => &["thread_id", "document_id", "entity_id", "relation_ord"],
            _ => &[],
        }
    }

    fn same_key(left: &Value, right: &Value, fields: &[&str]) -> bool {
        fields
            .iter()
