...
fn regression_lock_against_blessed_baseline() {
    ...
    let out = root.join("target").join("smoke").join("novel_full_regression");
    let baseline = root
        .join("tests")
        .join("baselines")
    ...
    let fresh = smoke::run_pipeline(&out, "regression", smoke::synthetic_novel());
    ...
    assert!(fresh.peak_rss_mib <= (base_rss as f32 * 1.25).ceil() as u64);
    assert!(fresh.query_pack_ms <= 15_000);

    let second = smoke::run_pipeline(&out, "regression_repeat", smoke::synthetic_novel());
...
fn adversarial_novel_variants_keep_structural_bones() {
    ...
    for (name, input) in adversarial_variants() {
        ...
        assert_eq!(first.chunk_hashes, second.chunk_hashes, "{name}: chunks drifted");
        assert_eq!(first.primary_cells, second.primary_cells, "{name}: projection drifted");
        assert_eq!(first.trace_hashes, second.trace_hashes, "{name}: cone trace drifted");
...
fn adversarial_variants() -> Vec<(&'static str, String)> {
    let clean = smoke::synthetic_novel();
    vec![
        ("novel_clean", clean.clone()),
        ("novel_with_frontmatter", format!("---\ntitle: Smoke\nlang: en\n---\n\n{clean}")),
        ("novel_with_weird_unicode", clean.replace("storm", "storm - cafe naive resume facade")),
        ("novel_with_blank_sections", clean.replace("\n\n", "\n\n\n\n\n")),
        ("novel_with_repeated_chapters", repeated_chapters()),
        ("novel_with_extreme_dialogue", clean.replace('.', ".\n\"No.\" \"Yes.\" \"Why?\" \"Because.\"")),
        ("novel_with_appendices", format!("{clean}\n\nAPPENDIX A\n{}\nAPPENDIX B\n{}", clean_tail(), clean_tail())),
        ("novel_scrambled_sections", scrambled_sections(&clean)),
    ]
}

fn repeated_chapters() -> String {
    let chapter = "Chapter R. Kai, Eureka, and Echo replay the same rupture with small witness changes. ";
    let mut text = String::with_capacity(260_000);
    for i in 0..3_800 {
        text.push_str(chapter);
        text.push_str(&format!("Pass {i}: evidence lane {}, causal lane {}.\n", i % 17, i % 23));
    }
    text
}

fn clean_tail() -> String {
    smoke::synthetic_novel().chars().take(40_000).collect()
}
