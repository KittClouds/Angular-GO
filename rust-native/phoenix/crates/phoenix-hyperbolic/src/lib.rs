...
impl<M: MetricF32> HyperbolicHnswBuilder<M> {
    ...
    pub fn insert(&mut self, mut vector: Vec<f32>) {
        ...
        if let Some(entry_id) = self.entry_point {
            ...
            for l in (0..=level.min(self.max_level)).rev() {
                ...
                let selected = self.select_neighbors_heuristic(candidates, max_c);
                ...
                for nb_id in selected {
                    ...
                    if nb_conn_len > max_c {
                        ...
                        let new_selected = self.select_neighbors_heuristic(nb_cands, max_c);
    ...
    fn select_neighbors_heuristic(&self, candidates: Vec<Candidate>, max_conn: usize) -> Vec<u32> {
    ...
    pub fn search(&self, query: &[f32], k: usize, ef_search: usize) -> Vec<Candidate> {
        let Some(mut curr) = self.entry_point else {
            return Vec::new();
        };
        if k == 0 || ef_search == 0 || self.nodes.is_empty() {
            return Vec::new();
        }
        ...
        let mut curr_dist = self
            .metric
            .rank_eval(&q_proj, &self.nodes[curr as usize].vector);

        for level in (1..=self.max_level).rev() {
            let mut changed = true;
            while changed {
                changed = false;
                let Some(connections) = self.nodes[curr as usize].connections.get(level as usize)
                else {
                    break;
                };
                for &neighbor_id in connections {
                    let distance = self
                        .metric
                        .rank_eval(&q_proj, &self.nodes[neighbor_id as usize].vector);
        ...
        while let Some(std::cmp::Reverse(candidate)) = candidates.pop() {
            if candidate.dist > results.peek().unwrap().dist && results.len() >= ef_search {
                break;
            }

            let Some(connections) = self.nodes[candidate.id as usize].connections.first() else {
                continue;
            };
            for &neighbor_id in connections {
                if visited.insert(neighbor_id) {
                    let distance = self
                        .metric
                        .rank_eval(&q_proj, &self.nodes[neighbor_id as usize].vector);
                    if results.len() < ef_search || distance < results.peek().unwrap().dist {
                        candidates.push(std::cmp::Reverse(Candidate {
                            id: neighbor_id,
                            dist: distance,
                        }));
        ...
        let mut final_candidates = results.into_vec();
        for candidate in &mut final_candidates {
            candidate.dist = self
                .metric
                .eval(&q_proj, &self.nodes[candidate.id as usize].vector);
        }
