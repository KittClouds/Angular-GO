...
const WASM_HARNESS_BUILD_ENV = 'PHOENIX_WASM_HARNESS_BUILD';
...
export class PhoenixWasmHarness {
    ...
    static async create(options?: { release?: boolean }): Promise<PhoenixWasmHarness> {
        ...
        const wasmPath = PhoenixWasmHarness.wasmPath(release);
        if (!existsSync(wasmPath)) {
            if (PhoenixWasmHarness.shouldBuildFromEnv()) {
                ensurePhoenixWasmBuilt(release);
            } else {
                throw new Error(
                    `Phoenix WASM harness needs a prebuilt ${release ? 'release' : 'debug'} wasm at ${wasmPath}. ` +
                    `Set ${WASM_HARNESS_BUILD_ENV}=1 to let the harness build it explicitly.`,
                );
            }
        }
    ...
    static hasPrebuilt(options?: { release?: boolean }): boolean {
        return existsSync(PhoenixWasmHarness.wasmPath(options?.release ?? false));
    }

    static shouldBuildFromEnv(): boolean {
        return process.env[WASM_HARNESS_BUILD_ENV] === '1';
    }

    private static wasmPath(release: boolean): string {
        return path.join(
            workspaceRoot(),
            'rust',
            'phoenix',
