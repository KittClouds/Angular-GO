...
describe('AnalyticsPanelComponent search highlights', () => {
    ...
    let phoenixUiApiMock: {
        ...
        lineSearch: ReturnType<typeof vi.fn>;
    ...
    it('applies parsed search highlight terms when analytics search runs', async () => {
        phoenixUiApiMock.lineSearch.mockResolvedValue([{
            noteId: 'note-2',
            title: 'Second Note',
            lineNumber: 1,
            lineStart: 0,
            lineEnd: 12,
            lineText: 'red gold Kai',
            matches: [{ from: 0, to: 8, term: 'red gold', data: null }],
            before: [],
            after: [],
            score: 0.92,
            generation: 'test',
        ...
        expect(phoenixUiApiMock.lineSearch).toHaveBeenCalledWith('"red gold" Kai', 32);
    ...
    it('routes retrieval through semantic search when embeddings mode is selected', async () => {
        component.setRetrievalMode('semantic');
        phoenixUiApiMock.semanticSearch.mockResolvedValue([{ DocID: 'note-2', Score: 0.92 }]);

        await component.performSearch('"red gold" Kai');

        expect(phoenixUiApiMock.semanticSearch).toHaveBeenCalledWith('"red gold" Kai', 10);
        expect(phoenixUiApiMock.lineSearch).not.toHaveBeenCalled();
        expect(component.searchResults()).toEqual([
            {
                id: 'note-2',
                score: 0.92,
                title: 'Second Note',
                sourceLabel: 'Embeddings',
                routeLabel: 'semantic ANN · Jina v5 Nano 768d',
            },
        ]);
    });

    it('lets the panel switch between native semantic runner labels', async () => {
        component.setRetrievalMode('semantic');
        component.setSemanticModel('bge-small-rust');
        phoenixUiApiMock.semanticSearch.mockResolvedValue([{ DocID: 'note-2', Score: 0.76 }]);

        await component.performSearch('refuge');

        expect(component.semanticModelLabel()).toBe('BGE Small Rust 384d');
        expect(component.searchResults()[0]?.routeLabel).toBe('semantic ANN · BGE Small Rust 384d');
    });
    ...
    it('keeps the active search highlight in place when opening a result note', async () => {
        phoenixUiApiMock.lineSearch.mockResolvedValue([{
            ...
            lineEnd: 3,
            lineText: 'Kai',
            matches: [{ from: 0, to: 3, term: 'Kai', data: null }],
            before: [],
            after: [],
            score: 1,
            generation: 'test',
    ...
    it('adds the open note as a local result when live editor text matches but search returns nothing', async () => {
        phoenixUiApiMock.lineSearch.mockResolvedValue([]);
    ...
    it('still shows the open note local result when search throws', async () => {
        phoenixUiApiMock.lineSearch.mockRejectedValue(new Error('worker failed'));
