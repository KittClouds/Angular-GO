...
export function buildGraphRebuildEmbeddingTargets(
    ...
    for (const node of nodes) targets.push({ id: `embed:entity:${node.entityId}`, kind: 'entity', sourceId: node.entityId, entityId: node.entityId, label: node.label, text: [node.label, ...node.aliases].join(' '), evidenceIds: node.anchorIds });
    for (const anchor of anchors) targets.push({ id: `embed:anchor:${anchor.id}`, kind: 'anchor', sourceId: anchor.id, noteId: anchor.noteId, chunkId: anchor.chunkId, entityId: anchor.entityId, label: anchor.surface, text: anchor.surface, evidenceIds: [anchor.id] });
    ...
    for (const event of events) targets.push({ id: `embed:event:${event.id}`, kind: 'event', sourceId: event.id, noteId: event.noteId, chunkId: event.chunkId, entityId: event.entityIds[0], label: event.label, text: event.label, evidenceIds: event.evidenceAnchorIds });
...
function chunkText(input: BuildGraphRebuildSnapshotInput, chunk: GraphRebuildChunk): string {
    ...
    return slice || `${chunk.noteId}:${chunk.start}-${chunk.end}`;
}
